...
public class MemberController {
	...
	public ResponseEntity<OutputMessage> registerMemberHandler(@RequestBody Member member) throws MemberException{
		return new ResponseEntity<OutputMessage>(memberService.registerMember(member), HttpStatus.CREATED);
	}
	
	@PostMapping("/login")
	public ResponseEntity<Member> loginMemberHandler(@RequestBody LoginDTO login) throws MemberException{
		return new ResponseEntity<Member>(memberService.loginMember(login), HttpStatus.OK);
	}
	
	@GetMapping("/getMyProjects/{memberId}")
	public ResponseEntity<List<Project>> getAllMyProjectsHandler(@PathVariable("memberId") Integer memberId) throws MemberException{
	...
	public ResponseEntity<List<Invites>> getAllMyInvitesHandler(@PathVariable("memberId") Integer memberId) throws MemberException{
